...
class GoogleDriveAddon(CloudDriveAddon):
    ...
    def _get_url_original(self, driveid, item_driveid=None, item_id=None):
        ...
        url += "|Authorization=%s" % urllib.parse.quote(
            "Bearer %s" % self._provider.get_access_tokens()['access_token'])
        return url
    ...
    def _select_stream_format(self, driveid, item_driveid=None, item_id=None, auto=False):
        ...
        request = Request('https://drive.google.com/get_video_info', urllib.parse.urlencode({'docid': item_id}),
    ...
    def get_context_options(self, list_item, params, is_folder):
        ...
        if Utils.get_safe_value(params, 'action', '') == 'play':
            ...
            context_options.append((self._addon.getLocalizedString(32071),
                                    'RunPlugin(' + self._addon_url + '?' + urllib.parse.urlencode(p) + ')'))
