...
class GoogleDrive(Provider):
    ...
    def __init__(self, source_mode=False):
        ...
        self._items_cache = Cache(KodiUtils.get_addon_info('id'), 'items',
    ...
    def configure(self, account_manager, driveid):
        ...
        self._is_shared_drive = drive and 'type' in drive and (
                drive['type'] == 'drive#drive' or drive['type'] == 'drive#teamDrive')
    ...
    def get_account(self, request_params=None, access_tokens=None):
        me = self.get('/about', parameters={'fields': 'user'}, request_params=request_params,
    ...
    def get_drives(self, request_params=None, access_tokens=None):
        ...
        try:
            ...
            while not all_shareddrives_fetch:
                ...
                response = self.get('/drives', parameters=parameters, request_params=request_params,
                                    access_tokens=access_tokens)
    ...
    def get_folder_items(self, item_driveid=None, item_id=None, path=None, on_items_page_completed=None,
                         include_download_info=False, on_before_add_item=None):
    ...
    def process_files(self, files, parameters, on_items_page_completed=None, include_download_info=False,
                      extra_info=None, on_before_add_item=None):
        items = []
        if files:
        ...
        if include_download_info:
            if is_media_items:
                ...
                if 'size' not in f and item['mimetype'] == 'application/vnd.google-apps.document':
                    url += '/export'
                    parameters['mimeType'] = Utils.default(Utils.get_mimetype_by_extension(item['name_extension']),
                                                           Utils.get_mimetype_by_extension('pdf'))
    ...
    def get_item_by_path(self, path, include_download_info=False):
        ...
        if not item:
            raise RequestException('Not found by path', HTTPError(path, 404, 'Not found', None, None),
    ...
    def changes(self):
        ...
        if not change_token:
            change_token = Utils.get_safe_value(
                self.get('/changes/startPageToken', parameters=self.prepare_parameters()), 'startPageToken')
        extra_info = {}
        parameters = self.prepare_parameters()
        parameters['pageToken'] = change_token
        parameters['pageSize'] = 1000
        parameters[
            'fields'] = 'kind,nextPageToken,newStartPageToken,changes(kind,fileId,removed,changeType,time,file(%s))' % self._get_field_parameters()
